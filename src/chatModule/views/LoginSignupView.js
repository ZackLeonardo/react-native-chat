...
export default class LoginSignupView extends LoggedView {
  ...
  onPressGithub = () => {
    ...
    const endpoint = `https://github.com/login?client_id=${clientId}&return_to=${encodeURIComponent(
  ...
  onPressTwitter = () => {
    ...
    const url = `${
      this.props.server
    }/_oauth/twitter/?requestTokenAndRedirect=true&state=${state}`;
    this.openOAuth(url);
  };
  ...
  getOAuthState = () => {
    ...
    return Base64.encodeURI(
      JSON.stringify({ loginStyle: "popup", credentialToken, isCordova: true })
    );
  };
    ...
    return (
      <View style={styles.servicesContainer}>
        ...
        <View style={sharedStyles.loginOAuthButtons} key="services">
          ...
          this.props.services["meteor-developer"] ? (
            <TouchableOpacity
