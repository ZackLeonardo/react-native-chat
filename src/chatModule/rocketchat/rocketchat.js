...
import { setActiveUser } from "../redux/actions/activeUsers";
import {
  starredMessagesReceived,
  starredMessageUnstarred
} from "../redux/actions/starredMessages";
import {
  pinnedMessagesReceived,
  pinnedMessageUnpinned
} from "../redux/actions/pinnedMessages";
import { mentionedMessagesReceived } from "../redux/actions/mentionedMessages";
import { snippetedMessagesReceived } from "../redux/actions/snippetedMessages";
import { roomFilesReceived } from "../redux/actions/roomFiles";
import { someoneTyping, roomMessageReceived } from "../redux/actions/room";
import { setRoles } from "../redux/actions/roles";
import Ddp from "../../main/ran-ddp/ddp";
...
import { getDeviceToken } from "../../main/ran-push";
...
const RocketChat = {
  ...
  subscribeRoom,
  canOpenRoom,
  createChannel({ name, users, type, readOnly, broadcast }) {
    return call(
  ...
  _hasInstanceId(headers) {
    return (
      (headers["x-instance-id"] != null &&
        headers["x-instance-id"].length > 0) ||
      (headers["X-Instance-ID"] != null && headers["X-Instance-ID"].length > 0)
    );
  },
  async testServer(url) {
    try {
      ...
      if (
        response.status === 200 &&
        RocketChat._hasInstanceId(response.headers)
      ) {
        return url;
      }
    } catch (e) {
  ...
  connect(url, login) {
    return new Promise(resolve => {
      ...
      this.ddp.on(
        "login",
        protectedFunction(() => reduxStore.dispatch(loginRequest()))
      );
      ...
      this.ddp.on(
        "forbidden",
        protectedFunction(() => reduxStore.dispatch(logout()))
      );
      ...
      this.ddp.on("background", () =>
        this.getRooms().catch(e => log("background getRooms", e))
      );

      this.ddp.on(
        "logged",
        protectedFunction(user => {
      ...
      this.ddp.on("stream-room-messages", ddpMessage => {
        ...
        requestAnimationFrame(() =>
          reduxStore.dispatch(roomMessageReceived(message))
        );
      });
        ...
        protectedFunction(ddpMessage => {
          ...
          if (ev === "typing") {
            reduxStore.dispatch(
              someoneTyping({
                _rid,
                username: ddpMessage.fields.args[0],
                typing: ddpMessage.fields.args[1]
              })
            );
        ...
        protectedFunction(ddpMessage => {
          if (ddpMessage.msg === "added") {
            ...
            this.starredMessagesTimer = setTimeout(
              protectedFunction(() => {
                reduxStore.dispatch(
                  starredMessagesReceived(this.starredMessages)
                );
                this.starredMessagesTimer = null;
          ...
          if (ddpMessage.msg === "removed") {
            if (reduxStore.getState().starredMessages.isOpen) {
              return reduxStore.dispatch(
                starredMessageUnstarred(ddpMessage.id)
              );
            }
          }
        })
        ...
        protectedFunction(ddpMessage => {
          if (ddpMessage.msg === "added") {
            ...
            this.mentionedMessagesTimer = setTimeout(() => {
              reduxStore.dispatch(
                mentionedMessagesReceived(this.mentionedMessages)
              );
              this.mentionedMessagesTimer = null;
        ...
        protectedFunction(ddpMessage => {
          if (ddpMessage.msg === "added") {
            ...
            this.snippetedMessagesTimer = setTimeout(() => {
              reduxStore.dispatch(
                snippetedMessagesReceived(this.snippetedMessages)
              );
              this.snippetedMessagesTimer = null;
      ...
      this.ddp.on(
        ...
        protectedFunction(ddpMessage => {
          if (ddpMessage.msg === "added") {
            ...
            this.loginServices[ddpMessage.fields.service] = {
              ...ddpMessage.fields
            };
            ...
            this.loginServiceTimer = setTimeout(
              () => reduxStore.dispatch(removeLoginServices()),
              1000
            );
          }
        })
      ...
      this.ddp.on(
        ...
        protectedFunction(ddpMessage => {
          ...
          this.roles[ddpMessage.id] =
            (ddpMessage.fields && ddpMessage.fields.description) || undefined;
        })
      );
  ...
  parseSettings: settings =>
    settings.reduce((ret, item) => {
      ret[item._id] =
        item[defaultSettings[item._id].type] ||
        item.valueAsString ||
        item.valueAsNumber ||
        item.valueAsBoolean ||
        item.value;
  ...
  _filterSettings: settings =>
    settings.filter(setting => defaultSettings[setting._id] && setting.value),
  parseEmojis: emojis =>
    emojis.reduce((ret, item) => {
  ...
  async getRoomMember(rid, currentUserId) {
    try {
      ...
      return Promise.resolve(
        membersResult.records.find(m => m.id !== currentUserId)
      );
    } catch (error) {
  ...
  hasPermission(permissions, rid) {
    ...
    const userRoles =
      (reduxStore.getState().login.user &&
        reduxStore.getState().login.user.roles) ||
      [];
    // merge both roles
    ...
    return permissions.reduce((result, permission) => {
      ...
      const permissionFound = permissionsFiltered.find(
        p => p._id === permission
      );
      if (permissionFound) {
